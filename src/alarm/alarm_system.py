...
class AlarmSystem:
    ...
    def trigger_alarm(self, alarm_type):
        ...
        # 记录报警类型
        if alarm_type not in self.alarm_confirmed:
            self.alarm_confirmed[alarm_type] = False

    def confirm_alarm(self, alarm_type):
        """
        确认报警
        ...
        Args:
            alarm_type: 报警类型
        """
        self.alarm_confirmed[alarm_type] = True
        logging.info(f"Alarm confirmed: {alarm_type}")
